...
from my_torchani.aev import SpeciesAEV
...
def neighbor_pairs(padding_mask: Tensor, coordinates: Tensor, cell: Tensor,
    ...
    in_cutoff = torch.nonzero(distances <= cutoff, as_tuple=False) # ffr
...
def compute_aev(species: Tensor, coordinates: Tensor, triu_index: Tensor,
    ...
    # Rca is usually much smaller than Rcr, using neighbor list with cutoff=Rcr
    # is a waste of resources
    # Now we will get a smaller neighbor list that only cares about atoms
    # with distances <= Rca
    even_closer_indices = (distances <= Rca).nonzero().flatten()
    #even_closer_indices = torch.nonzero(distances <= Rca, as_tuple=False).flatten() # ffr
    print("even_closer_indices = ", even_closer_indices)
